...
contract Strategy is AMMStrategyBase {
    // ITERATION_POLICY {"decision":"continue","hypothesis_id":"H_TOX_ACT_011","confidence":0.78,"ceiling_probability":0.39,"ev_next_5":0.04,"best_delta_seen":2.3074,"reason":"Selected H_TOX_ACT_011 on eff_priority 0.968; promotion_edge 519.26 and T2 favor capped adverse-side tox/activity widening in transition stress.","next_mechanism":"toxicity_and_activity"}
    ...
    uint256 constant TOX_QUAD_ADD_CAP = 450 * BPS;
    uint256 constant TOX_FAVORABLE_MULT = 860000000000000000;
    uint256 constant TOX_ADVERSE_MULT = 1120000000000000000;
    uint256 constant ACT_COEF = 42000 * BPS;
    uint256 constant ACT_ADVERSE_SHARE = 220000000000000000;
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        if (prevLambdaSignal >= ACT_GATE_LAMBDA && prevSizeSignal >= ACT_GATE_SIZE && prevToxSignal > TOX_QUAD_KNEE) {
        ...
        uint256 actAdd = 0;
        if (lambdaHat >= ACT_GATE_LAMBDA && sizeHat >= ACT_GATE_SIZE && toxSignal > TOX_QUAD_KNEE) {
            actAdd = wmul(ACT_COEF, actEma);
        }

        uint256 toxActAdd = toxFavAdd + actAdd;
        uint256 prevToxActAdd = prevToxFavAdd + prevActAdd;
        ...
        uint256 actCappedAdd = toxActAdd > toxFavAdd ? toxActAdd - toxFavAdd : 0;
        uint256 actSplitAdd = wmul(actCappedAdd, ACT_ADVERSE_SHARE);
    ...
    function getName() external pure override returns (string memory) {
        return "toxicity_and_activity_mod_v248";
    }
}
