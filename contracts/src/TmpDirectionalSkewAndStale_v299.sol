...
contract Strategy is AMMStrategyBase {
    // ITERATION_POLICY {"decision":"continue","hypothesis_id":"H_DIR_STALE_005","confidence":0.56,"ceiling_probability":0.30,"ev_next_5":0.019,"best_delta_seen":2.3074,"reason":"Selected H_DIR_STALE_005 on eff_priority 0.99 with steps_since_last 20; T10 blends reserve-imbalance and flow with deadbanded asymmetry while preserving promotion_edge 519.30 discipline.","next_mechanism":"directional_skew_and_stale_protection"}
    ...
    uint256 constant DIR_INV_FLOW_WEIGHT = 860000000000000000;
    uint256 constant DIR_SKEW_DEADBAND = 2 * BPS;
    uint256 constant DIR_ATTRACT_FRAC = 900000000000000000;
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        bool invSellPressure = spot >= pHat;
        uint256 invDev = 0;
        uint256 spotPlusPHat = spot + pHat;
        if (spotPlusPHat > 0) {
            invDev = wdiv(absDiff(spot, pHat), spotPlusPHat);
        }

        uint256 dirWeighted = wmul(dirDev, DIR_INV_FLOW_WEIGHT);
        uint256 invWeighted = wmul(invDev, WAD - DIR_INV_FLOW_WEIGHT);

        bool skewSellPressure;
        uint256 skewDev;
        if (sellPressure == invSellPressure) {
            skewSellPressure = sellPressure;
            skewDev = dirWeighted + invWeighted;
        } else if (dirWeighted >= invWeighted) {
            skewSellPressure = sellPressure;
            skewDev = dirWeighted - invWeighted;
        } else {
            skewSellPressure = invSellPressure;
            skewDev = invWeighted - dirWeighted;
        }

        uint256 skewCore = skewDev > DIR_SKEW_DEADBAND ? skewDev - DIR_SKEW_DEADBAND : 0;
        uint256 skew = wmul(DIR_COEF, skewCore) + wmul(DIR_TOX_COEF, wmul(skewCore, toxSignal));
        uint256 attractSkew = wmul(skew, DIR_ATTRACT_FRAC);

        uint256 bidFee;
        uint256 askFee;
        if (skewSellPressure) {
            bidFee = fMid + skew;
            askFee = fMid > attractSkew ? fMid - attractSkew : 0;
        } else {
            askFee = fMid + skew;
            bidFee = fMid > attractSkew ? fMid - attractSkew : 0;
        }
        ...
        if (STALE_DIR_COEF > 0 && tox > TOX_QUAD_KNEE) {
            uint256 staleShift = wmul(STALE_DIR_COEF, tox - TOX_QUAD_KNEE);
            if (staleShift > TOX_QUAD_ADD_CAP) staleShift = TOX_QUAD_ADD_CAP;
            uint256 staleAttractShift = wmul(staleShift, DIR_ATTRACT_FRAC);
            if (spot >= pHat) {
                bidFee = bidFee + staleShift;
                askFee = askFee > staleAttractShift ? askFee - staleAttractShift : 0;
            } else {
                askFee = askFee + staleShift;
                bidFee = bidFee > staleAttractShift ? bidFee - staleAttractShift : 0;
            }
        }
    ...
    function getName() external pure override returns (string memory) {
        return "directional_skew_and_stale_protection_mod_v299";
    }
}
