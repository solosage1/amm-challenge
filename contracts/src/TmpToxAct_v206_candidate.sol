...
contract Strategy is AMMStrategyBase {
    // ITERATION_POLICY {"decision":"continue","hypothesis_id":"H_TOX_ACT_008","confidence":0.62,"ceiling_probability":0.31,"ev_next_5":0.09,"best_delta_seen":2.3074,"reason":"Scale activity widening down as toxicity rises above the tox knee to avoid stress-period double widening while retaining calm-flow activity response.","next_mechanism":"toxicity_and_activity"}
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        if (lambdaHat >= ACT_GATE_LAMBDA && sizeHat >= ACT_GATE_SIZE) {
            uint256 actScale = WAD;
            if (toxSignal > TOX_QUAD_KNEE) {
                uint256 toxSpan = TOX_CAP - TOX_QUAD_KNEE;
                uint256 toxOver = toxSignal - TOX_QUAD_KNEE;
                if (toxOver > toxSpan) toxOver = toxSpan;
                actScale = WAD - wdiv(toxOver, toxSpan);
            }
            actAdd = wmul(wmul(ACT_COEF, actEma), actScale);
        }
    ...
    function getName() external pure override returns (string memory) {
        return "toxicity_and_activity_mod_v206";
    }
}
