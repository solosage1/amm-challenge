...
contract Strategy is AMMStrategyBase {
    // ITERATION_POLICY {"decision":"continue","hypothesis_id":"H_TOX_ACT_005","confidence":0.58,"ceiling_probability":0.34,"ev_next_5":0.11,"best_delta_seen":2.3074,"reason":"Cap tox fee input only when sigma is elevated to limit stale/noisy overreaction while retaining baseline tox defense.","next_mechanism":"toxicity_and_activity"}
    ...
    uint256 constant TOX_CONF_SIGMA_GATE = 40 * BPS;
    uint256 constant TOX_CONF_CAP = 120 * BPS;
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        uint256 toxFeeSignal = toxSignal;
        if (sigmaHat >= TOX_CONF_SIGMA_GATE && toxFeeSignal > TOX_CONF_CAP) {
            toxFeeSignal = TOX_CONF_CAP;
        }

        uint256 toxExcess = toxFeeSignal > TOX_QUAD_KNEE ? toxFeeSignal - TOX_QUAD_KNEE : 0;
        uint256 actAdd = 0;
        if (lambdaHat >= ACT_GATE_LAMBDA && sizeHat >= ACT_GATE_SIZE) {
            actAdd = wmul(ACT_COEF, actEma);
        }
        uint256 fMid = fBase + wmul(TOX_COEF, toxFeeSignal) + wmul(TOX_QUAD_COEF, wmul(toxExcess, toxExcess)) + actAdd;
    ...
    function getName() external pure override returns (string memory) {
        return "toxicity_and_activity_mod_v168";
    }
}
