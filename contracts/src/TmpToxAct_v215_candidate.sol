...
contract Strategy is AMMStrategyBase {
    // ITERATION_POLICY {"decision":"continue","hypothesis_id":"H_TOX_ACT_001","confidence":0.64,"ceiling_probability":0.35,"ev_next_5":0.06,"best_delta_seen":0.15,"reason":"Test low-try H_TOX_ACT_001 (tries=1, mean_delta=-0.0327) to avoid recent H_TOX_ACT_009 concentration.","next_mechanism":"toxicity_and_activity"}
    ...
    uint256 constant TOX_SIDE_SHIFT = 300000000000000000;
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        uint256 fMid = fBase + wmul(TOX_COEF, toxSignal) + wmul(TOX_QUAD_COEF, wmul(toxExcess, toxExcess)) + actAdd;
        ...
        uint256 toxActivityAdd = fMid > fBase ? fMid - fBase : 0;
        uint256 toxSideShift = wmul(TOX_SIDE_SHIFT, toxActivityAdd);
        if (spot >= pHat) {
            bidFee = bidFee + toxSideShift;
            askFee = askFee > toxSideShift ? askFee - toxSideShift : 0;
        } else {
            askFee = askFee + toxSideShift;
            bidFee = bidFee > toxSideShift ? bidFee - toxSideShift : 0;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "toxicity_and_activity_mod_v215";
    }
}
