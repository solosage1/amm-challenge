...
contract Strategy is AMMStrategyBase {
    // ITERATION_POLICY {"decision":"continue","hypothesis_id":"H_TOX_ACT_011","confidence":0.57,"ceiling_probability":0.33,"ev_next_5":0.08,"best_delta_seen":0.15,"reason":"Highest shortlist priority (0.968) with only 1 try and mild mean_delta -0.0087; test tox-knee-gated adverse-side activity widening to reduce lock-in after H_TOX_ACT_009 repeat streak.","next_mechanism":"toxicity_and_activity"}
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        uint256 actAdd = 0;
        if (toxSignal >= TOX_QUAD_KNEE && lambdaHat >= ACT_GATE_LAMBDA && sizeHat >= ACT_GATE_SIZE) {
            actAdd = wmul(ACT_COEF, actEma);
        }
        uint256 fMid = fBase + wmul(TOX_COEF, toxSignal) + wmul(TOX_QUAD_COEF, wmul(toxExcess, toxExcess));
        ...
        if (actAdd > 0) {
            if (spot >= pHat) {
                bidFee = bidFee + actAdd;
            } else {
                askFee = askFee + actAdd;
            }
        }
    ...
    function getName() external pure override returns (string memory) {
        return "toxicity_and_activity_mod_v212";
    }
}
