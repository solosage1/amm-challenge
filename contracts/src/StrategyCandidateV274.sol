...
contract Strategy is AMMStrategyBase {
    // ITERATION_POLICY {"decision":"continue","hypothesis_id":"H_TOX_ACT_003","confidence":0.67,"ceiling_probability":0.23,"ev_next_5":0.08,"best_delta_seen":2.3074,"reason":"H_TOX_ACT_003 selected on eff_priority 0.740 with steps_since_last 20; apply T5 uncertainty-aware tox softening while defending promotion_edge 519.29","next_mechanism":"toxicity_and_activity"}
    ...
    uint256 constant TOX_SIGMA_SOFT_KNEE = 12 * BPS;
    uint256 constant TOX_SIGMA_SOFT_SPAN = 40 * BPS;
    uint256 constant TOX_SIGMA_SOFT_MIN_MULT = 760000000000000000;
    ...
    uint256 constant TOX_QUAD_ADD_CAP = 180 * BPS;
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        prevToxQuadAdd = wmul(prevToxQuadAdd, _toxSigmaSoftMult(sigmaHat));
        ...
        toxQuadAdd = wmul(toxQuadAdd, _toxSigmaSoftMult(sigmaHat));
        ...
        if (toxActAdd > prevToxActAdd) {
            ...
            if (drop > TOX_ACT_STEP_DOWN_CAP) toxActAdd = prevToxActAdd - TOX_ACT_STEP_DOWN_CAP;
        }
    ...
    function _toxSigmaSoftMult(uint256 sigmaHat) internal pure returns (uint256) {
        if (sigmaHat <= TOX_SIGMA_SOFT_KNEE) return WAD;
        uint256 sigmaExcess = sigmaHat - TOX_SIGMA_SOFT_KNEE;
        if (sigmaExcess > TOX_SIGMA_SOFT_SPAN) sigmaExcess = TOX_SIGMA_SOFT_SPAN;
        return WAD - wmul(WAD - TOX_SIGMA_SOFT_MIN_MULT, wdiv(sigmaExcess, TOX_SIGMA_SOFT_SPAN));
    }

    function getName() external pure override returns (string memory) {
        return "toxicity_and_activity_mod_v274";
    }
}
