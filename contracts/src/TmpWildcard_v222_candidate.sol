...
contract Strategy is AMMStrategyBase {
    // ITERATION_POLICY {"decision":"continue","hypothesis_id":"H_WILDCARD_222","confidence":0.71,"ceiling_probability":0.24,"ev_next_5":0.13,"best_delta_seen":0.18,"reason":"Pivot to continuous dual-anchor and inventory-aware directional skew to reduce calm over-widening while preserving shock defense.","next_mechanism":"dual_anchor_inventory_surface"}
    ...
    uint256 constant TOXVAR_COEF = 8000 * BPS;
    uint256 constant DISAGREE_COEF = 13000 * BPS;
    uint256 constant BURST_COEF = 11000 * BPS;
    uint256 constant JUMP_COEF = 9000 * BPS;
    uint256 constant TWOSIDE_COEF = 2500 * BPS;
    uint256 constant INV_MID_COEF = 3500 * BPS;

    uint256 constant DIR_FLOW_COEF = 95 * BPS;
    uint256 constant DIR_TOX_COEF = 26 * BPS;
    uint256 constant DIR_DISAGREE_COEF = 60 * BPS;
    uint256 constant INV_SKEW_COEF = 85 * BPS;
    uint256 constant TREND_SKEW_COEF = 65 * BPS;
    uint256 constant STALE_SHIFT_COEF = 5600 * BPS;
    ...
    uint256 constant TAIL_KNEE_DISAGREE_COEF = 1800 * BPS;
    uint256 constant TAIL_KNEE_JUMP_COEF = 1800 * BPS;
    ...
    uint256 constant RATE_UP_MIN = 36 * BPS;
    uint256 constant RATE_UP_RISK_RANGE = 170 * BPS;
    uint256 constant RATE_DOWN_MIN = 24 * BPS;
    ...
    function _computeMidFee(SwapState memory s) internal pure returns (uint256 fMid, uint256 volMix) {
        ...
        fMid = BASE_FEE
            ...
            + wmul(INV_MID_COEF, s.invSkewEma);
    }
    ...
    function _riskNorm(SwapState memory s, uint256 volMix) internal pure returns (uint256) {
        uint256 risk = s.toxEma + volMix + s.disagreeEma + s.toxVar + s.jumpEma + wmul(HALF_WAD, s.invSkewEma);
