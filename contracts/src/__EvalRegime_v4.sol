...
contract Strategy is AMMStrategyBase {
    ...
    uint256 constant ACT_DECAY = 780000000000000000; // 0.70
    ...
    uint256 constant LAMBDA_DECAY = 992000000000000000; // 0.99
    uint256 constant SIZE_BLEND_DECAY = 850000000000000000; // 0.65
    uint256 constant TOX_BLEND_DECAY = 100000000000000000;
    uint256 constant ACT_BLEND_DECAY = 995000000000000000;
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        if (isNewStep) {
            ...
            // Update arrival-rate estimate with elapsed-aware blending for sparse/bursty steps.
            if (elapsedRaw > 0) {
                uint256 lambdaElapsed = elapsedRaw > ELAPSED_CAP ? ELAPSED_CAP : elapsedRaw;
                uint256 lambdaDecay = _powWad(LAMBDA_DECAY, lambdaElapsed);
                if (stepTradeCount > 0) {
