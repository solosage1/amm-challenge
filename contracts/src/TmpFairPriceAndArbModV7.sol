...
contract Strategy is AMMStrategyBase {
    // ITERATION_POLICY {"decision":"continue","confidence":0.78,"ceiling_probability":0.47,"ev_next_5":0.86,"best_delta_seen":0.63,"reason":"Adaptive fair-price gating and arb classification should reduce pHat contamination while preserving quick arb anchoring.","next_mechanism":"fair_price_and_arb"}
    ...
    uint256 constant PHAT_ALPHA_ARB = 420000000000000000;
    uint256 constant PHAT_ALPHA_RETAIL = 80000000000000000;
    uint256 constant PHAT_SHOCK_GATE = 160 * BPS;
    uint256 constant PHAT_SHOCK_GATE_MIN = 90 * BPS;
    uint256 constant PHAT_SHOCK_GATE_MAX = 400 * BPS;
    uint256 constant PHAT_SHOCK_SIGMA_COEF = 5000000000000000000; // 5.0
    uint256 constant DIR_IMPACT_MULT = 1;
    uint256 constant ARB_MAX_RATIO = WAD / 360;
    uint256 constant ARB_MAX_RATIO_CAP = WAD / 170;
    uint256 constant ARB_SIGMA_RATIO_COEF = 3000000000000000000; // 3.0
    uint256 constant ARB_RET_GATE_MULT = 1400000000000000000; // 1.4
    uint256 constant IMPLIED_GAMMA_FLOOR = WAD / 4;
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        uint256 gammaRaw = feeUsed < WAD ? WAD - feeUsed : 0;
        uint256 gammaSafe = gammaRaw;
        if (gammaSafe < IMPLIED_GAMMA_FLOOR) gammaSafe = IMPLIED_GAMMA_FLOOR;

        uint256 pImpliedFee = trade.isBuy ? wmul(spot, gammaSafe) : wdiv(spot, gammaSafe);
        uint256 pImplied = wmul(pImpliedFee, gammaRaw) + wmul(spot, WAD - gammaRaw);
        ...
        if (firstInStep) {
            uint256 shockGate = PHAT_SHOCK_GATE + wmul(PHAT_SHOCK_SIGMA_COEF, sigmaHat);
            if (shockGate < PHAT_SHOCK_GATE_MIN) shockGate = PHAT_SHOCK_GATE_MIN;
            if (shockGate > PHAT_SHOCK_GATE_MAX) shockGate = PHAT_SHOCK_GATE_MAX;

            uint256 arbRatioGate = ARB_MAX_RATIO + wmul(ARB_SIGMA_RATIO_COEF, sigmaHat);
            if (arbRatioGate > ARB_MAX_RATIO_CAP) arbRatioGate = ARB_MAX_RATIO_CAP;

            uint256 ret = pHat > 0 ? wdiv(absDiff(pImplied, pHat), pHat) : 0;
            uint256 arbRetGate = wmul(shockGate, ARB_RET_GATE_MULT);
            if (arbRetGate < PHAT_SHOCK_GATE_MIN) arbRetGate = PHAT_SHOCK_GATE_MIN;
            if (arbRetGate > RET_CAP) arbRetGate = RET_CAP;

            bool likelyArb = tradeRatio <= arbRatioGate && ret <= arbRetGate;
            if (likelyArb) {
                if (ret <= shockGate) {
    ...
    function getName() external pure override returns (string memory) {
        return "fair_price_and_arb_mod_v7";
    }
}
