...
contract Strategy is AMMStrategyBase {
    // ITERATION_POLICY {"decision":"continue","hypothesis_id":"H_REGIME_001","confidence":0.63,"ceiling_probability":0.24,"ev_next_5":0.026,"best_delta_seen":2.3074,"reason":"Selected H_REGIME_001 on eff_priority 0.982 at robust_edge 519.30; T8 favors elapsed-aware estimator continuity.","next_mechanism":"regime_estimators"}
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        if (isNewStep) {
            ...
            uint256 lambdaDecay = _powWad(LAMBDA_DECAY, elapsed);
            if (stepTradeCount > 0) {
                uint256 lambdaInst = (stepTradeCount * WAD) / elapsedRaw;
                if (lambdaInst > LAMBDA_CAP) lambdaInst = LAMBDA_CAP;
                lambdaHat = wmul(lambdaHat, lambdaDecay) + wmul(lambdaInst, WAD - lambdaDecay);
            } else {
                lambdaHat = wmul(lambdaHat, lambdaDecay);
            }
        ...
        if (firstInStep) {
            ...
            if (likelyArb) {
                ...
                uint256 sigmaDecay = SIGMA_DECAY;
                if (isNewStep) {
                    uint256 sigmaElapsedRaw = trade.timestamp - lastTs;
                    uint256 sigmaElapsed = sigmaElapsedRaw > ELAPSED_CAP ? ELAPSED_CAP : sigmaElapsedRaw;
                    sigmaDecay = _powWad(SIGMA_DECAY, sigmaElapsed);
                }
                sigmaHat = wmul(sigmaHat, sigmaDecay) + wmul(ret, WAD - sigmaDecay);
            } else {
                if (ret <= PHAT_SHOCK_GATE) {
                    pHat = wmul(pHat, WAD - PHAT_ALPHA_RETAIL) + wmul(pImplied, PHAT_ALPHA_RETAIL);
                }
                uint256 sigmaRetailDecay = SIGMA_RETAIL_DECAY;
                if (isNewStep) {
                    uint256 sigmaElapsedRaw = trade.timestamp - lastTs;
                    uint256 sigmaElapsed = sigmaElapsedRaw > ELAPSED_CAP ? ELAPSED_CAP : sigmaElapsedRaw;
                    sigmaRetailDecay = _powWad(SIGMA_RETAIL_DECAY, sigmaElapsed);
                }
                sigmaHat = wmul(sigmaHat, sigmaRetailDecay);
            }
        }
    ...
    function getName() external pure override returns (string memory) {
        return "regime_estimators_mod_v307";
    }
}
