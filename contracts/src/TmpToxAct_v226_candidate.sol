...
contract Strategy is AMMStrategyBase {
    // ITERATION_POLICY {"decision":"continue","hypothesis_id":"H_TOX_ACT_010","confidence":0.59,"ceiling_probability":0.31,"ev_next_5":0.08,"best_delta_seen":0.15,"reason":"Highest eff_priority=0.932 with risk_flags=none and steps_since_last=3; testing a per-trade clamp on tox/activity addend changes.","next_mechanism":"toxicity_and_activity"}
    ...
    uint256 constant TOX_ACT_DELTA_CAP = 12 * BPS;
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        uint256 prevFBase =
            BASE_FEE + wmul(SIGMA_COEF, slots[6]) + wmul(LAMBDA_COEF, slots[7]) + wmul(FLOW_SIZE_COEF, wmul(slots[7], slots[8]));
        ...
        uint256 toxActAdd = wmul(TOX_COEF, toxSignal) + wmul(TOX_QUAD_COEF, wmul(toxExcess, toxExcess)) + actAdd;
        uint256 prevMid = (prevBidFee + prevAskFee) / 2;
        uint256 prevToxActAdd = prevMid > prevFBase ? prevMid - prevFBase : 0;
        if (toxActAdd > prevToxActAdd + TOX_ACT_DELTA_CAP) {
            toxActAdd = prevToxActAdd + TOX_ACT_DELTA_CAP;
        } else if (prevToxActAdd > toxActAdd + TOX_ACT_DELTA_CAP) {
            toxActAdd = prevToxActAdd - TOX_ACT_DELTA_CAP;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "toxicity_and_activity_mod_v226";
    }
}
