...
contract Strategy is AMMStrategyBase {
    // ITERATION_POLICY {"decision":"continue","hypothesis_id":"H_FAIR_ARB_004","confidence":0.64,"ceiling_probability":0.27,"ev_next_5":0.018,"best_delta_seen":2.3074,"reason":"Selected on eff_priority 0.976; per T5 we sigma-weight pHat alpha while champion promotion_edge is 519.30.","next_mechanism":"fair_price_and_arb"}
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        if (firstInStep) {
            ...
            uint256 pHatConfMult = WAD;
            if (sigmaHat > TOX_CONF_SIGMA_KNEE) {
                uint256 sigmaExcess = sigmaHat - TOX_CONF_SIGMA_KNEE;
                if (sigmaExcess > TOX_CONF_SIGMA_SPAN) sigmaExcess = TOX_CONF_SIGMA_SPAN;
                pHatConfMult = WAD - wmul(WAD - TOX_CONF_MIN_MULT, wdiv(sigmaExcess, TOX_CONF_SIGMA_SPAN));
            }

            if (likelyArb) {
                uint256 alphaArb = wmul(PHAT_ALPHA_ARB, pHatConfMult);
                if (ret <= PHAT_SHOCK_GATE) {
                    pHat = wmul(pHat, WAD - alphaArb) + wmul(pImplied, alphaArb);
                }
                if (ret > RET_CAP) ret = RET_CAP;
                sigmaHat = wmul(sigmaHat, SIGMA_DECAY) + wmul(ret, WAD - SIGMA_DECAY);
            } else {
                uint256 alphaRetail = wmul(PHAT_ALPHA_RETAIL, pHatConfMult);
                if (ret <= PHAT_SHOCK_GATE) {
    ...
    function getName() external pure override returns (string memory) {
        return "fair_price_and_arb_mod_v300";
    }
}
