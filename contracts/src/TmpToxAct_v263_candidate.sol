...
contract Strategy is AMMStrategyBase {
    // ITERATION_POLICY {"decision":"continue","hypothesis_id":"H_TOX_ACT_011","confidence":0.68,"ceiling_probability":0.26,"ev_next_5":0.07,"best_delta_seen":2.3074,"reason":"eff_priority 0.928 and promotion_edge 519.26 support T1; calibrate adverse tox/activity widening by likelyArb state to target arb share <0.22.","next_mechanism":"toxicity_and_activity"}
    ...
    uint256 constant TOX_QUAD_ADD_CAP = 240 * BPS;
    ...
    uint256 constant TOX_ACT_ADVERSE_ARB_MULT = 1180000000000000000;
    uint256 constant TOX_ACT_ADVERSE_CALM_MULT = 880000000000000000;
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        if (toxActSplitAdd > 0) {
            uint256 adverseMult = likelyArb ? TOX_ACT_ADVERSE_ARB_MULT : TOX_ACT_ADVERSE_CALM_MULT;
            toxActSplitAdd = wmul(toxActSplitAdd, adverseMult);
        }
    ...
    function getName() external pure override returns (string memory) {
        return "toxicity_and_activity_mod_v265";
    }
}
