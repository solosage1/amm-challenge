...
contract Strategy is AMMStrategyBase {
    // ITERATION_POLICY {"decision":"continue","hypothesis_id":"H_DIR_STALE_002","confidence":0.58,"ceiling_probability":0.31,"ev_next_5":0.014,"best_delta_seen":2.3074,"reason":"Selected H_DIR_STALE_002 on eff_priority 0.960 with exploration_bonus 0.080 (tries 0) at promotion_edge 519.30; apply T2 same-step post-arb attract-side cut with bounded impact.","next_mechanism":"directional_skew_and_stale_protection"}
    ...
    uint256 constant POST_ARB_RETAIL_CUT = 4 * BPS;
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        if (likelyArb) {
            uint256 postArbCut = wmul(POST_ARB_RETAIL_CUT, WAD - toxSignal);
            if (bidFee <= askFee) {
                bidFee = bidFee > postArbCut ? bidFee - postArbCut : 0;
            } else {
                askFee = askFee > postArbCut ? askFee - postArbCut : 0;
            }
        }
    ...
    function getName() external pure override returns (string memory) {
        return "directional_skew_and_stale_protection_mod_v304";
    }
}
