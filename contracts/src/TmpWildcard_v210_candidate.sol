...
contract Strategy is AMMStrategyBase {
    // ITERATION_POLICY {"decision":"continue","hypothesis_id":"H_WILDCARD_210","confidence":0.69,"ceiling_probability":0.27,"ev_next_5":0.12,"best_delta_seen":0.15,"reason":"Shift to dual-anchor disagreement risk with two-sided flow imbalance and bounded adaptation to improve stale protection without over-widening in calm flow.","next_mechanism":"dual_anchor_disagreement_surface"}
    ...
    uint256 constant DISAGREE_DECAY = 870000000000000000;
    uint256 constant SIGMA_FAST_DECAY = 700000000000000000;
    uint256 constant SIGMA_SLOW_DECAY = 930000000000000000;
    uint256 constant LAMBDA_IDLE_DECAY = 960000000000000000;
    uint256 constant LAMBDA_BLEND_DECAY = 994000000000000000;
    ...
    uint256 constant SIGMA_FAST_BLEND = 700000000000000000;
    uint256 constant SIGMA_SLOW_BLEND = 960000000000000000;

    uint256 constant FAST_ALPHA_BASE = 80000000000000000;
    uint256 constant FAST_ALPHA_SIZE = 320000000000000000;
    uint256 constant FAST_ALPHA_DISAGREE = 100000000000000000;
    uint256 constant FAST_ALPHA_MAX = 360000000000000000;

    uint256 constant SLOW_ALPHA_BASE = 12000000000000000;
    uint256 constant SLOW_ALPHA_SIZE = 120000000000000000;
    uint256 constant SLOW_ALPHA_MAX = 140000000000000000;

    uint256 constant FAST_MOVE_CAP = 180 * BPS;
    uint256 constant SLOW_MOVE_CAP = 70 * BPS;

    uint256 constant RET_CAP = WAD / 10;
    uint256 constant TOX_CAP = WAD / 5;
    uint256 constant STALE_CAP = WAD / 5;
    ...
    uint256 constant SIGMA_COEF = 210000000000000000;
    uint256 constant LAMBDA_COEF = 11 * BPS;
    uint256 constant FLOW_SIZE_COEF = 7100 * BPS;

    uint256 constant TOX_COEF = 175 * BPS;
    uint256 constant TOX_QUAD_COEF = 17500 * BPS;
    uint256 constant TOX_QUAD_KNEE = 11 * BPS;
    uint256 constant TOXVAR_COEF = 9000 * BPS;
    uint256 constant DISAGREE_COEF = 14000 * BPS;

    uint256 constant BURST_COEF = 12000 * BPS;
    uint256 constant TWOSIDE_COEF = 4500 * BPS;

    uint256 constant DIR_COEF = 95 * BPS;
    uint256 constant DIR_TOX_COEF = 28 * BPS;
    uint256 constant DIR_DISAGREE_COEF = 70 * BPS;
    uint256 constant STALE_SHIFT_COEF = 6200 * BPS;

    uint256 constant TAIL_KNEE_BASE = 680 * BPS;
    uint256 constant TAIL_KNEE_TOX_COEF = 2200 * BPS;
    uint256 constant TAIL_KNEE_VOL_COEF = 1600 * BPS;
    uint256 constant TAIL_KNEE_DISAGREE_COEF = 2200 * BPS;
    uint256 constant TAIL_KNEE_MAX = 1300 * BPS;
    ...
    uint256 constant RATE_UP_MIN = 38 * BPS;
    uint256 constant RATE_UP_RISK_RANGE = 180 * BPS;
    uint256 constant RATE_DOWN_MIN = 25 * BPS;
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        (uint256 bidFee, uint256 askFee, uint256 volMix) = _rawQuote(s, spot);
        ...
        _storeState(s, ts, bidFee, askFee);
    ...
    function _decayOnNewStep(SwapState memory s, uint256 ts, uint256 lastTs) internal pure {
        ...
        s.buyFlow = wmul(s.buyFlow, _powWad(FLOW_DECAY, elapsed));
        s.sellFlow = wmul(s.sellFlow, _powWad(FLOW_DECAY, elapsed));
    ...
    function _updateFlowState(SwapState memory s, bool isBuy, uint256 tradeRatio) internal pure {
        ...
        if (isBuy) {
            s.buyFlow = wmul(s.buyFlow, FLOW_BLEND_DECAY) + wmul(tradeRatio, WAD - FLOW_BLEND_DECAY);
            s.sellFlow = wmul(s.sellFlow, FLOW_BLEND_DECAY);
        } else {
            s.sellFlow = wmul(s.sellFlow, FLOW_BLEND_DECAY) + wmul(tradeRatio, WAD - FLOW_BLEND_DECAY);
            s.buyFlow = wmul(s.buyFlow, FLOW_BLEND_DECAY);
        }
    }

    function _updatePriceState(SwapState memory s, uint256 spot, uint256 pImplied, uint256 tradeRatio) internal pure {
        _updateSigmaState(s, pImplied);
        _updateAnchorState(s, pImplied, tradeRatio);
        _updateToxicityState(s, spot);
    }

    function _updateSigmaState(SwapState memory s, uint256 pImplied) internal pure {
    ...
    function _updateAnchorState(SwapState memory s, uint256 pImplied, uint256 tradeRatio) internal pure {
    ...
    function _updateToxicityState(SwapState memory s, uint256 spot) internal pure {
    ...
    function _computeMidFee(SwapState memory s) internal pure returns (uint256 fMid, uint256 volMix) {
        ...
        fMid = BASE_FEE
            ...
            + wmul(TOXVAR_COEF, s.toxVar)
            + wmul(DISAGREE_COEF, s.disagreeEma)
            + wmul(BURST_COEF, s.burstEma)
            + wmul(TWOSIDE_COEF, twoSidedFlow);
    }

    function _flowImbalance(SwapState memory s) internal pure returns (uint256 imbalance, bool sellPressure) {
        uint256 flowTotal = s.buyFlow + s.sellFlow + BPS;
        uint256 flowDiff;

        if (s.buyFlow >= s.sellFlow) {
            flowDiff = s.buyFlow - s.sellFlow;
            sellPressure = true;
        } else {
            flowDiff = s.sellFlow - s.buyFlow;
            sellPressure = false;
        }
    ...
    function _rawQuote(
        ...
        (fMid, volMix) = _computeMidFee(s);
        (imbalance, sellPressure) = _flowImbalance(s);
        (bidFee, askFee) = _quoteFees(s, spot, volMix, fMid, imbalance, sellPressure);
    }
    ...
    function _quoteFees(
        ...
        uint256 skew = wmul(DIR_COEF, imbalance)
        ...
        if (stale > STALE_CAP) stale = STALE_CAP;
    ...
    function _riskNorm(SwapState memory s, uint256 volMix) internal pure returns (uint256) {
        uint256 riskNorm = s.toxEma + volMix + s.disagreeEma + s.toxVar;
