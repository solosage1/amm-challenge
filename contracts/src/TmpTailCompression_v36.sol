...
contract Strategy is AMMStrategyBase {
    // ITERATION_POLICY {"decision":"continue","hypothesis_id":"H_TAIL_004","confidence":0.58,"ceiling_probability":0.28,"ev_next_5":0.19,"best_delta_seen":0.0385,"reason":"Test per-trade fee jump caps before tail compression to smooth spikes and reduce starvation while preserving defense.","next_mechanism":"tail_compression"}
    ...
    uint256 constant TAIL_STEP_MAX_PROTECT = 80 * BPS;
    uint256 constant TAIL_STEP_MAX_ATTRACT = 50 * BPS;
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        if (bidFee > askFee) {
            bidFee = _limitFeeJump(prevBidFee, bidFee, TAIL_STEP_MAX_PROTECT);
            askFee = _limitFeeJump(prevAskFee, askFee, TAIL_STEP_MAX_ATTRACT);
            ...
            bidFee = _limitFeeJump(prevBidFee, bidFee, TAIL_STEP_MAX_ATTRACT);
            askFee = _limitFeeJump(prevAskFee, askFee, TAIL_STEP_MAX_PROTECT);
            bidFee = clampFee(_compressTail(bidFee, TAIL_SLOPE_ATTRACT));
            askFee = clampFee(_compressTail(askFee, TAIL_SLOPE_PROTECT));
        } else {
            bidFee = _limitFeeJump(prevBidFee, bidFee, (TAIL_STEP_MAX_PROTECT + TAIL_STEP_MAX_ATTRACT) / 2);
            askFee = _limitFeeJump(prevAskFee, askFee, (TAIL_STEP_MAX_PROTECT + TAIL_STEP_MAX_ATTRACT) / 2);
    ...
    function _limitFeeJump(uint256 prevFee, uint256 nextFee, uint256 maxJump) internal pure returns (uint256) {
        if (nextFee > prevFee) {
            uint256 up = nextFee - prevFee;
            return up > maxJump ? prevFee + maxJump : nextFee;
        }

        uint256 down = prevFee - nextFee;
        if (down > maxJump) {
            return prevFee > maxJump ? prevFee - maxJump : 0;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "tail_compression_mod_v36";
    }
}
