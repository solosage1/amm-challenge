...
contract Strategy is AMMStrategyBase {
    // ITERATION_POLICY {"decision":"continue","hypothesis_id":"H_TOX_ACT_001","confidence":0.60,"ceiling_probability":0.30,"ev_next_5":0.11,"best_delta_seen":2.3074,"reason":"Test adverse-side-only toxicity widening to reduce safe-side overpricing while preserving protection on the vulnerable side.","next_mechanism":"toxicity_and_activity"}
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        if (spot >= pHat) {
            bidFee = bidFee + toxAdd;
        } else {
            askFee = askFee + toxAdd;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "toxicity_and_activity_mod_v176";
    }
}
