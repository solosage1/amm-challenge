...
contract Strategy is AMMStrategyBase {
    // ITERATION_POLICY {"decision":"continue","hypothesis_id":"H_TOX_ACT_001","confidence":0.57,"ceiling_probability":0.30,"ev_next_5":0.09,"best_delta_seen":0.15,"reason":"Chosen for eff_priority=0.844 with steps_since_last=6 and repeat_streak=0; use a new favorable-side tox/activity discount while keeping adverse-side widening intact.","next_mechanism":"toxicity_and_activity"}
    ...
    uint256 constant TOX_FAVORABLE_SHARE = 780000000000000000;
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        uint256 actAdd = wmul(TOX_COEF, toxSignal) + wmul(TOX_QUAD_COEF, wmul(toxExcess, toxExcess));
        if (lambdaHat >= ACT_GATE_LAMBDA && sizeHat >= ACT_GATE_SIZE) {
            actAdd = actAdd + wmul(ACT_COEF, actEma);
        }
        toxExcess = actAdd - wmul(actAdd, TOX_FAVORABLE_SHARE);
        uint256 fMid = fBase + (actAdd - toxExcess);
        ...
        if (spot >= pHat) {
            bidFee = bidFee + toxExcess;
        } else {
            askFee = askFee + toxExcess;
        }
    ...
    function getName() external pure override returns (string memory) {
        return "toxicity_and_activity_mod_v229";
    }
}
