...
contract Strategy is AMMStrategyBase {
    ...
    // New tight-band flow-memory knobs.
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (isNewStep) {
            ...
            uint256 gamma = trade.isBuy ? (WAD - prevBid) : (WAD - prevAsk); // mock mutation iter=2 mechanism=fair_price_estimation
        ...
        if (ry != 0 && FLOW_ALPHA_PCT > 0) { // mock mutation iter=3 mechanism=flow_memory // mock mutation iter=7 mechanism=flow_memory
        ...
        if (mis <= bpsToWad(TIGHT_BAND_BPS)) { // mock mutation iter=9 mechanism=tight_band_pricing
        ...
        } else { // mock mutation iter=8 mechanism=outer_regime_pricing
    ...
    function getName() external pure override returns (string memory) {
        return "tight_band_pricing_mod_v9";
    }
}

// wildcard mutation iter=10

```

## YOUR TASK
Modify the **flow_memory** mechanism to improve expected edge.

### Current Implementation
Exponential flow score update with decay, impulse cap, and clamp.

### Code Location
afterSwap lines 130-147
...
- Use multi-timescale flow tracking
- Add regime reset on large moves
- Weight momentum by trade size
- Infer queue pressure from imbalance

## CONSTRAINTS
1. ONLY modify code related to flow_memory
2. Keep all other mechanisms unchanged:
fair_price_estimation, arbitrage_response, tight_band_pricing, outer_regime_pricing
3. Output a complete, compilable Solidity contract
4. Keep contract declaration as `contract Strategy`
5. Set getName() return value to "flow_memory_mod_v11"
