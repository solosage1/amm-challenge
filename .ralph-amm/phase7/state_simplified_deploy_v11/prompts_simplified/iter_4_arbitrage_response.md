...
contract Strategy is AMMStrategyBase {
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (ry != 0 && FLOW_ALPHA_PCT > 0) { // mock mutation iter=3 mechanism=flow_memory
...
## YOUR TASK
Modify the **arbitrage_response** mechanism to improve expected edge.

### Current Implementation
Timestamp arb tagging with post-arb fee cut in same timestamp.

### Code Location
afterSwap lines 157-163, 213-217
...
- Add gradual recovery curve after arb
- Use direction-asymmetric arb response
- Scale cut size by recent arb frequency
- Use size-aware arb response

## CONSTRAINTS
1. ONLY modify code related to arbitrage_response
2. Keep all other mechanisms unchanged:
fair_price_estimation, flow_memory, tight_band_pricing, outer_regime_pricing
3. Output a complete, compilable Solidity contract
4. Keep contract declaration as `contract Strategy`
5. Set getName() return value to "arbitrage_response_mod_v4"
