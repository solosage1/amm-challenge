...
contract Strategy is AMMStrategyBase {
    ...
    // 14 stressScoreBps (EWMA)
    // 15 uncertaintyScoreBps (EWMA)
    // 16 fallbackHoldUntilTimestamp
    // 17 lastShockTimestampPlusOne
    ...
    // Additive robustness layer (canary family).
    uint256 private constant RB_STRESS_DECAY_PCT = 81;
    uint256 private constant RB_STRESS_CAP_BPS = 220;
    uint256 private constant RB_SHOCK_CAP_BPS = 160;
    uint256 private constant RB_SHOCK_HARD_BPS = 72;
    uint256 private constant RB_SHOCK_ARB_BONUS_BPS = 14;
    uint256 private constant RB_JUMP_WEIGHT_PCT = 65;
    uint256 private constant RB_MIS_WEIGHT_PCT = 35;
    uint256 private constant RB_LOSS_TRIGGER_BPS = 9;
    uint256 private constant RB_LOSS_BONUS_CAP_BPS = 60;
    uint256 private constant RB_REPLAY_STEPS = 2;
    uint256 private constant RB_REPLAY_BONUS_BPS = 11;

    uint256 private constant RB_UNCERT_DECAY_PCT = 85;
    uint256 private constant RB_UNCERT_CAP_BPS = 260;
    uint256 private constant RB_UNCERT_STRESS_WEIGHT_PCT = 58;
    uint256 private constant RB_UNCERT_CONF_WEIGHT_PCT = 42;
    uint256 private constant RB_UNCERT_ENTER_BPS = 126;
    uint256 private constant RB_UNCERT_EXIT_BPS = 56;
    uint256 private constant RB_STRESS_ENTER_BPS = 132;
    uint256 private constant RB_HOLD_STEPS = 1;

    uint256 private constant RB_SAFE_TIGHT_ADD_BPS = 1;
    uint256 private constant RB_SAFE_BUFFER_ADD_CENTI_BPS = 25;
    uint256 private constant RB_SAFE_COMP_CUT_CENTI_BPS = 10;
    uint256 private constant RB_FALLBACK_FEE_CENTI_BPS = 2978;
    uint256 private constant RB_FALLBACK_MIS_MAX_BPS = 24;
    ...
    function tradeLossBps(uint256 fairPrev, uint256 amountX, uint256 amountY, bool isBuy)
        internal
        pure
        returns (uint256)
    {
        ...
        if (isBuy) {
            if (amountY > ref) return wdiv(amountY - ref, ref) / BPS;
            return 0;
        }
    ...
    function updateRobustState(
        uint256 stress,
        uint256 uncertainty,
        uint256 fallbackUntil,
        uint256 lastShockTsP1,
        uint256 ts,
        uint256 jumpBps,
        uint256 misBps,
        uint256 lossBps,
        ...
        internal
        pure
        returns (uint256 nextStress, uint256 nextUncertainty, uint256 nextFallbackUntil, uint256 nextLastShockTsP1)
    {
        uint256 shock = (jumpBps * RB_JUMP_WEIGHT_PCT + misBps * RB_MIS_WEIGHT_PCT) / 100;
        if (arbLike) shock += RB_SHOCK_ARB_BONUS_BPS;

        if (lossBps > RB_LOSS_TRIGGER_BPS) {
            uint256 lossBonus = lossBps - RB_LOSS_TRIGGER_BPS;
            if (lossBonus > RB_LOSS_BONUS_CAP_BPS) lossBonus = RB_LOSS_BONUS_CAP_BPS;
            shock += lossBonus;
        }

        if (lastShockTsP1 != 0) {
            uint256 lastShockTs = lastShockTsP1 - 1;
            if (ts <= lastShockTs + RB_REPLAY_STEPS) {
                shock += RB_REPLAY_BONUS_BPS;
            }
        }
        if (shock > RB_SHOCK_CAP_BPS) shock = RB_SHOCK_CAP_BPS;

        stress = (stress * RB_STRESS_DECAY_PCT + shock * (100 - RB_STRESS_DECAY_PCT)) / 100;
        if (stress > RB_STRESS_CAP_BPS) stress = RB_STRESS_CAP_BPS;
        if (shock >= RB_SHOCK_HARD_BPS) {
            lastShockTsP1 = ts + 1;
        }

        uint256 confDef = conf >= 10000 ? 0 : (10000 - conf);
        uint256 confDefScaled = confDef / 100;
        uint256 uncertInput =
            (stress * RB_UNCERT_STRESS_WEIGHT_PCT + confDefScaled * RB_UNCERT_CONF_WEIGHT_PCT) / 100;
        uncertainty = (uncertainty * RB_UNCERT_DECAY_PCT + uncertInput * (100 - RB_UNCERT_DECAY_PCT)) / 100;
        if (uncertainty > RB_UNCERT_CAP_BPS) uncertainty = RB_UNCERT_CAP_BPS;

        if (uncertainty >= RB_UNCERT_ENTER_BPS || stress >= RB_STRESS_ENTER_BPS) {
            fallbackUntil = ts + RB_HOLD_STEPS;
        } else if (fallbackUntil < ts && uncertainty <= RB_UNCERT_EXIT_BPS) {
            fallbackUntil = 0;
        }

        nextStress = stress;
        nextUncertainty = uncertainty;
        nextFallbackUntil = fallbackUntil;
        nextLastShockTsP1 = lastShockTsP1;
    }
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 stress = slots[14];
        uint256 uncertainty = slots[15];
        uint256 fallbackUntil = slots[16];
        uint256 lastShockTs = slots[17];
        if (aggr == 0) aggr = WAD;
        if (risk == 0) risk = WAD;
        if (conf == 0) conf = 5000;
        ...
        if (spot == 0 || fair == 0) {
            ...
            slots[2] = askFee;
            slots[14] = stress;
            slots[15] = uncertainty;
            slots[16] = fallbackUntil;
            slots[17] = lastShockTs;
        ...
        if (isNewStep) {
            uint256 jumpBps = 0;
            if (lastSpot != 0 && spot != 0) {
                jumpBps = wdiv(absDiff(spot, lastSpot), lastSpot) / BPS;
            }

            uint256 lossBps = tradeLossBps(fairPrev, trade.amountX, trade.amountY, trade.isBuy);
            (stress, uncertainty, fallbackUntil, lastShockTs) = updateRobustState(
                stress,
                uncertainty,
                fallbackUntil,
                lastShockTs,
        ...
        if (uncertainty >= RB_UNCERT_ENTER_BPS && activeArm == 2) activeArm = 1;
        bool stressMode = stress >= RB_STRESS_ENTER_BPS || uncertainty >= RB_UNCERT_ENTER_BPS;
        bool fallbackMode = fallbackUntil != 0 && trade.timestamp <= fallbackUntil;
        ...
        if (mis <= bpsToWad(tightBandBps)) {
            ...
            if (stressMode && RB_SAFE_TIGHT_ADD_BPS > 0) {
                uint256 add = bpsToWad(RB_SAFE_TIGHT_ADD_BPS);
                bidFee = clampFee(bidFee + add);
                askFee = clampFee(askFee + add);
            }
            ...
            if (stressMode) {
                buffer += centiBpsToWad(RB_SAFE_BUFFER_ADD_CENTI_BPS);
                uint256 soften = centiBpsToWad(RB_SAFE_COMP_CUT_CENTI_BPS);
                undercut = undercut > soften ? (undercut - soften) : 0;
            }
        ...
        if (fallbackMode && mis <= bpsToWad(RB_FALLBACK_MIS_MAX_BPS)) {
            uint256 fallbackFee = centiBpsToWad(RB_FALLBACK_FEE_CENTI_BPS);
            bidFee = fallbackFee;
            askFee = fallbackFee;
        }
