...
contract Strategy is AMMStrategyBase {
    ...
    // 10 sideStreakLen
    // 11 latentRetailBeliefBps (0..10000; higher means retail-capture regime)
    // 12 latentMode (0 neutral, 1 retail-capture, 2 defensive)
    ...
    // Inventory and streak asymmetry layer.
    uint256 private constant INV_DEADBAND_BPS = 9;
    uint256 private constant INV_DIV = 8;
    uint256 private constant INV_MAX_BPS = 3;

    uint256 private constant STREAK_ENTER = 4;
    uint256 private constant STREAK_FLIP_BPS = 3;
    uint256 private constant STREAK_BONUS_MAX_BPS = 2;

    uint256 private constant ALIGN_COMP_BONUS_PCT = 45;
    uint256 private constant MISALIGN_COMP_CUT_PCT = 55;
    uint256 private constant ALIGN_PROTECT_CUT_PCT = 10;
    uint256 private constant MISALIGN_PROTECT_BONUS_PCT = 45;
    uint256 private constant BIAS_COMP_MAX_BPS = 2;
    uint256 private constant BIAS_PROTECT_MAX_BPS = 2;
    uint256 private constant TIGHT_INV_TILT_MAX_BPS = 1;

    // Latent two-state filter.
    uint256 private constant LATENT_UP_BPS = __LATENT_UP_BPS__;
    uint256 private constant LATENT_DOWN_BPS = __LATENT_DOWN_BPS__;
    uint256 private constant LATENT_DRIFT_BPS = __LATENT_DRIFT_BPS__;
    uint256 private constant RETAIL_ENTER_BPS = __RETAIL_ENTER_BPS__;
    uint256 private constant DEFENSE_ENTER_BPS = __DEFENSE_ENTER_BPS__;
    uint256 private constant RETAIL_EXIT_BPS = __RETAIL_EXIT_BPS__;
    uint256 private constant DEFENSE_EXIT_BPS = __DEFENSE_EXIT_BPS__;
    uint256 private constant MODE_TIGHT_SHIFT_BPS = __MODE_TIGHT_SHIFT_BPS__;
    uint256 private constant MODE_UNDERCUT_SHIFT_BPS = __MODE_UNDERCUT_SHIFT_BPS__;
    uint256 private constant MODE_BUFFER_SHIFT_BPS = __MODE_BUFFER_SHIFT_BPS__;
    uint256 private constant MODE_POST_ARB_CUT_BPS = __MODE_POST_ARB_CUT_BPS__;
    uint256 private constant EVIDENCE_MIS_BPS = __EVIDENCE_MIS_BPS__;
    uint256 private constant EVIDENCE_QUIET_BPS = __EVIDENCE_QUIET_BPS__;
    ...
    function inventoryBiasState(uint256 rx, uint256 ry, uint256 fair)
        internal
        pure
        returns (uint256 side, uint256 bps)
    {
        if (rx == 0 || fair == 0) return (0, 0);

        uint256 xValueInY = wmul(rx, fair);
        if (xValueInY == 0) return (0, 0);

        uint256 invRatio = wdiv(ry, xValueInY);
        bps = centeredTiltBps(invRatio, INV_DEADBAND_BPS, INV_DIV, INV_MAX_BPS);
        if (bps == 0) return (0, 0);

        side = invRatio >= WAD ? 1 : 2;
    }
    ...
    function edgePositive(TradeInfo calldata trade, uint256 fair) internal pure returns (bool) {
        if (fair == 0) return false;
        uint256 ref = wmul(trade.amountX, fair);
        if (trade.isBuy) {
            return trade.amountY >= ref;
        }
        return ref >= trade.amountY;
    }

    function updateLatentMode(
        uint256 belief,
        uint256 mode,
        bool arbLike,
        bool retailLike,
        bool edgePos
    ) internal pure returns (uint256 nextBelief, uint256 nextMode) {
        uint256 center = 5000;
        if (belief > center) {
            uint256 over = belief - center;
            belief -= (over * LATENT_DRIFT_BPS) / 10000;
        } else if (belief < center) {
            uint256 under = center - belief;
            belief += (under * LATENT_DRIFT_BPS) / 10000;
        }

        if (arbLike) {
            belief = belief > LATENT_DOWN_BPS ? (belief - LATENT_DOWN_BPS) : 0;
        } else {
            uint256 up = 0;
            if (retailLike) up += LATENT_UP_BPS;
            if (edgePos) up += LATENT_UP_BPS / 2;
            if (up > 0) {
                belief = belief + up;
                if (belief > 10000) belief = 10000;
            }
        }

        if (mode == 1 && belief < RETAIL_EXIT_BPS) mode = 0;
        if (mode == 2 && belief > DEFENSE_EXIT_BPS) mode = 0;

        if (mode == 0) {
            if (belief >= RETAIL_ENTER_BPS) mode = 1;
            else if (belief <= DEFENSE_ENTER_BPS) mode = 2;
        } else if (mode == 1 && belief <= DEFENSE_ENTER_BPS) {
            mode = 2;
        } else if (mode == 2 && belief >= RETAIL_ENTER_BPS) {
            mode = 1;
        }
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        uint256 belief = slots[11];
        uint256 mode = slots[12];
        if (belief == 0) belief = 5000;
        ...
        bool arbLike = toward && mis >= bpsToWad(EVIDENCE_MIS_BPS);
        bool retailLike = (!toward) && mis <= bpsToWad(EVIDENCE_QUIET_BPS);
        bool edgePos = edgePositive(trade, fair);
        if (isNewStep && toward && mis >= bpsToWad(ARB_TAG_MIS_BPS)) {
            slots[5] = trade.timestamp;
        }
        (belief, mode) = updateLatentMode(belief, mode, arbLike, retailLike, edgePos);
        slots[11] = belief;
        slots[12] = mode;
        ...
        (uint256 invSide, uint256 invBps) = inventoryBiasState(rx, ry, fair);
        uint256 bidBias = invSide == 1 ? invBps : 0;
        uint256 askBias = invSide == 2 ? invBps : 0;
        if (streak >= STREAK_ENTER) {
            uint256 streakBps = streak - STREAK_ENTER + 1;
            if (streakBps > STREAK_BONUS_MAX_BPS) streakBps = STREAK_BONUS_MAX_BPS;
            if (lastSide == 1) bidBias += streakBps;
            else if (lastSide == 2) askBias += streakBps;
        }
        ...
        if (bidBias > askBias) {
            biasSide = 1;
            biasBps = bidBias - askBias;
        } else if (askBias > bidBias) {
            biasSide = 2;
            biasBps = askBias - bidBias;
        }

        if (streak >= STREAK_ENTER && biasSide != 0 && biasSide != lastSide && biasBps < STREAK_FLIP_BPS) {
            biasSide = lastSide;
            biasBps = STREAK_FLIP_BPS;
        }

        if (mis <= bpsToWad(TIGHT_BAND_BPS)) {
            uint256 tight = centiBpsToWad(TIGHT_FEE_CENTI_BPS);
            uint256 modeTightShift = bpsToWad(MODE_TIGHT_SHIFT_BPS);
            if (mode == 1) {
                tight = tight > modeTightShift ? (tight - modeTightShift) : 0;
            } else if (mode == 2) {
                tight = clampFee(tight + modeTightShift);
            }
            bidFee = tight;
            askFee = tight;
            ...
            uint256 invTightBps = invBps;
            if (invTightBps > TIGHT_INV_TILT_MAX_BPS) invTightBps = TIGHT_INV_TILT_MAX_BPS;
            if (invTightBps > 0) {
            ...
            uint256 modeUndercutShift = bpsToWad(MODE_UNDERCUT_SHIFT_BPS);
            uint256 modeBufferShift = bpsToWad(MODE_BUFFER_SHIFT_BPS);
            if (mode == 1) {
                undercut += modeUndercutShift;
                buffer = buffer > modeBufferShift ? (buffer - modeBufferShift) : 0;
            } else if (mode == 2) {
                undercut = undercut > modeUndercutShift ? (undercut - modeUndercutShift) : 0;
                buffer += modeBufferShift;
            }
        ...
        if (mode == 1) {
            postArbCutBps += MODE_POST_ARB_CUT_BPS;
        } else if (mode == 2) {
            postArbCutBps = postArbCutBps > MODE_POST_ARB_CUT_BPS ? (postArbCutBps - MODE_POST_ARB_CUT_BPS) : 0;
        }
