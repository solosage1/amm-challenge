...
contract Strategy is AMMStrategyBase {
    ...
    uint256 private constant DYN_CALM_MIS_BPS = 40;
    uint256 private constant DYN_STRESS_MIS_BPS = 90;
    uint256 private constant DYN_CALM_UNDERCUT_ADD_CENTI_BPS = 60;
    uint256 private constant DYN_CALM_BUFFER_CUT_CENTI_BPS = 30;
    uint256 private constant DYN_STRESS_UNDERCUT_CUT_CENTI_BPS = 120;
    uint256 private constant DYN_STRESS_BUFFER_ADD_CENTI_BPS = 80;
        ...
        returns (uint256 bidFee, uint256 askFee)
    {
        ...
        if (mis <= bpsToWad(TIGHT_BAND_BPS)) {
            ...
            if (mis <= bpsToWad(DYN_CALM_MIS_BPS)) {
                undercut += centiBpsToWad(DYN_CALM_UNDERCUT_ADD_CENTI_BPS);
                uint256 calmCut = centiBpsToWad(DYN_CALM_BUFFER_CUT_CENTI_BPS);
                buffer = buffer > calmCut ? (buffer - calmCut) : 0;
            } else if (mis >= bpsToWad(DYN_STRESS_MIS_BPS)) {
                uint256 stressCut = centiBpsToWad(DYN_STRESS_UNDERCUT_CUT_CENTI_BPS);
                undercut = undercut > stressCut ? (undercut - stressCut) : 0;
                buffer += centiBpsToWad(DYN_STRESS_BUFFER_ADD_CENTI_BPS);
            }
